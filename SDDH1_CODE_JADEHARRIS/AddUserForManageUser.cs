...
{
    public partial class frm_addUserForManageUser : Form
    {

        frm_manageUsers manageUsersForm;

        public frm_addUserForManageUser(frm_manageUsers manageUsersCallingForm) //Create a form reference so that functions from Manage Users can be called
        {
            manageUsersForm = manageUsersCallingForm;
            InitializeComponent();
        }
        ...
        private void txt_generate_Click(object sender, EventArgs e) //Generate a password if the user selects the button
        {
            GeneratePassword();
        }
        ...
        {
            //Ensure that the fieds are complete and notify the user if one is empty
            ...
            else
            {
                ...
                if (usernameUnique == true) //If the username is unique
                {
                    ...
                    else //Else here means the password must be greater 6 chaaracters
                    {
                        ...
                        //Update the manage users grid with the new user
                        manageUsersForm.ReadSubjectOverview();
        ...
        private void FillStageDependingOnSubject()
        {
            if (cmb_subject.SelectedIndex != -1) //As this event triggers whenever the subject index is changed (even if that means it is empty), avoid errors by ensuring a subject has be chosen
            {
        ...
        private void CheckIfSubjectandStageActive()
        {
